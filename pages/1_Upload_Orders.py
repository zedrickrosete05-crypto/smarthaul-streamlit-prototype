...
# ---------- Time parsing (supports AM/PM + 24h + Excel) ----------
_AMPM = re.compile(
    r"""^\s*(?P<h>\d{1,2})(?:
            [:\u2236](?P<m>\d{2}) | (?P<m2>\d{2})?
        )\s*(?P<ampm>a\.?m\.?|p\.?m\.?)\s*$""",
...
def to_minutes(val: Any) -> Optional[int]:
    ...
    if m:
        ...
        if 0 <= h < 24 and 0 <= mnt < 60: return h * 60 + mnt
    m = _H24.match(s)
...
def geocode_unique(df: pd.DataFrame, suffix=" Philippines", delay_s: float = 1.0):
...
with st.expander("📁 Download CSV template"):
    ...
    st.caption("Accepted formats: '08:30 AM', '1:15pm', '14:00', Excel time cells.")

# ---------- Upload ----------
uploaded = st.file_uploader("Upload Orders CSV", type=["csv", "xlsx"])
...
st.download_button("⬇️ Download cleaned orders (CSV)",
                   df.to_csv(index=False).encode("utf-8"),
                   "orders_cleaned.csv", "text/csv")

st.info("Next: go to **Optimize Routes** to plan deliveries.")
