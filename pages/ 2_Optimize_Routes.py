...
# Uses OSM Nominatim (respect usage policy, 1 req/sec, cache results)
try:
    from geopy.geocoders import Nominatim
    from geopy.extra.rate_limiter import RateLimiter

    _geolocator = Nominatim(user_agent="smarthaul-demo")
    _reverse = RateLimiter(_geolocator.reverse, min_delay_seconds=1)

    @st.cache_data(show_spinner=False, ttl=60 * 60 * 24)
    def reverse_geocode(lat: float, lon: float) -> str:
        try:
            loc = _reverse((lat, lon), language="en", zoom=15)
            if loc:
                # Take a short form (house/POI, road, barangay/city)
...
if "routes_df" in st.session_state:
    ...
    # Optional: resolve place names
    use_places = st.checkbox("Show place names (reverse-geocode)", value=False,
                             help="Adds a 'Place' column and labels using OpenStreetMap Nominatim (1 req/sec, cached).")
    if use_places and GEOCODER_OK:
        with st.spinner("Resolving place names..."):
            df["place"] = df.apply(lambda r: reverse_geocode(float(r["lat"]), float(r["lon"])), axis=1)
    elif use_places and not GEOCODER_OK:
        st.info("Install geopy in requirements.txt to enable place names:  geopy==2.4.1")
    ...
    cols = ["Vehicle", "Stop #", "Order", "ETA", "Time window", "Alert", "Lat", "Lon"]
    display_df = df.rename(
        columns={"vehicle_id": "Vehicle", "order_id": "Order", "eta": "ETA", "alert": "Alert"}
    )
    if "place" in df.columns:
        display_df = display_df.assign(Place=df["place"])
        cols = ["Vehicle", "Stop #", "Order", "ETA", "Time window", "Alert", "Place", "Lat", "Lon"]
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Place": st.column_config.TextColumn("Place", help="Short place name (reverse-geocoded)"),
    ...
    if show_map:
        try:
            ...
            if use_places and "place" in df.columns:
            ...
            # Label: "V1-1" or "V1-1: Place"
            if "place" in df_map.columns:
